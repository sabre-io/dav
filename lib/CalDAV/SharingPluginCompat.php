...
/**
 * This plugin implements calendar sharing, but provides backwards
 * compatibility for an older spec.
 *
 ...
 */
class SharingPluginCompat extends DAV\ServerPlugin {
    ...
    function initialize(DAV\Server $server) {
        ...
        $server->xml->elementMap['{' . Plugin::NS_CALENDARSERVER . '}share'] = 'Sabre\\CalDAV\\Xml\\Request\\Share';
        $server->xml->elementMap['{' . Plugin::NS_CALENDARSERVER . '}invite-reply'] = 'Sabre\\CalDAV\\Xml\\Request\\InviteReply';

        $server->on('propFind',     [$this,'propFindEarly']);
        $server->on('propFind',     [$this,'propFindLate'], 150);
        $server->on('propPatch',    [$this, 'propPatch'], 40);
