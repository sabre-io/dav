...
class Sabre_DAV_DebugPlugin extends Sabre_DAV_ServerPlugin {
    ...
    function __construct($logFile) {
        ...
        if (!is_writeable($logFile)) {
            throw new Sabre_DAV_Exception('Unable to write to: ' . $logFile);
        }
    ...
    public function initialize(Sabre_DAV_Server $server) {
        ...
        $this->log('Initialized plugin. Request time ' . $this->startTime . ' (' . date(DateTime::RFC2822,$this->startTime) . '). Version: ' . Sabre_DAV_Version::VERSION);
    ...
    public function beforeMethod($method, $uri) {
        ...
        $this->log('beforeMethod triggered. Method: ' . $method . ' uri: ' . ($uri?$uri:'(root)'));
        $this->log('Plugins loaded:');
        foreach($this->server->getPlugins() as $pluginName => $plugin) {
            $this->log('  ' . $pluginName . ' (' . get_class($plugin) . ')');
        }
        $this->log('SabreDAV server Base URI: ' . $this->server->getBaseUri()); 
        $this->log('HTTP Request:');
        $this->log($this->server->httpRequest->getMethod() . ' ' . $this->server->httpRequest->getUri());
        $this->log(print_r($this->server->httpRequest->getHeaders(),true));
        ...
        if (!$showBody) {
            $this->log('Request body Content-Type does not match our whitelist, so it is not displayed here');
    ...
    public function unknownMethod($method, $uri) {
        ...
        $this->log('unknownMethod triggered. Method: ' . $method . ' uri: ' . ($uri?$uri:'(root)'));
    ...
    public function report($reportName) {
        ...
        $this->log('Report requested: ' . $reportName);
    ...
    public function beforeGetProperties($path, Sabre_DAV_INode $node, $requestedProperties, $returnedProperties) {
        ...
        $this->log('Properties requested for uri (' . $path . '):');
        $this->log(print_r($requestedProperties,true));
